...
export default function CreateHousePage() {
  ...
  const validateForm = () => {
    ...
    // Parking spaces validation (0-20)
    if (!formData.parkingSpaces) {
      ...
      if (isNaN(parkingSpaces) || parkingSpaces < 0 || parkingSpaces > 20) {
        newErrors.parkingSpaces = "จำนวนที่จอดรถต้องเป็นตัวเลข 0-20";
      }
    }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const houseData = {
      ...
      landArea: formData.landArea ? parseFloat(formData.landArea) : 0,
