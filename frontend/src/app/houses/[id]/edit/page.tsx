...
import { useHouse, useUpdateHouse } from '@/hooks/api/useHouses'
...
export default function EditHousePage() {
  ...
  const { data: house, isLoading: loading, error } = useHouse(houseId)
  ...
  // Check ownership and populate form when house data is loaded
  useEffect(() => {
    if (house && user) {
      // Check if current user is the owner or admin
      const canEdit = house.ownerId === user?.id || user?.role === 'ADMIN' || user?.role === 'MODERATOR'
      
      if (!canEdit) {
        toast.error('คุณไม่มีสิทธิ์แก้ไขประกาศนี้')
        router.push(`/houses/${houseId}`)
        return
      }
      
      setIsOwner(house.ownerId === user?.id)
      setFormData({
        title: house.title || '',
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    updateHouseMutation.mutate(
      { id: houseId, data: houseData },
      {
        onSuccess: () => {
          // Add timestamp to force refresh when returning to detail page
          router.push(`/houses/${houseId}?_t=${Date.now()}`)
        }
      }
    )
  }
      ...
      <div className="flex items-center justify-between mb-6">
        <div className="flex items-center space-x-4">
          ...
          <div>
            ...
            <p className="text-gray-600 dark:text-gray-400">แก้ไขข้อมูลประกาศขายบ้านของคุณ</p>
          </div>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        {updateHouseMutation.error && (
          <div className="bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800 text-red-700 dark:text-red-400 px-4 py-3 rounded-lg">
            {updateHouseMutation.error.message}
          </div>
        )}
...
{/* Location */}
        ...
        <div className="flex justify-end space-x-4">
          <Button type="button" variant="outline" asChild>
            <Link href={`/houses/${houseId}`}>ยกเลิก</Link>
          </Button>
